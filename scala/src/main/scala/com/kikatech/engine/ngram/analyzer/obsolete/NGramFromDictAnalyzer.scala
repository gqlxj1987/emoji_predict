...
/**
  * Created by huminghe on 2017/7/4.
  */
class NGramFromDictAnalyzer extends Analyzer {
  ...
  override def doAnalyze(sparkContext: SparkContext, paramArray: Array[String]): Unit = {
    ...
    val dict2Weight = 0.2
    val dict3Weight = 0.2
    val originWeight = 0.2

    val onlineWeight = 0.6
    val twitterWeight = 0.4
    ...
    val uniGramOutputPath = paramArray(11)
    val biGramOutputPath = paramArray(12)
    val partitionNum = paramArray(13).toInt
  ...
  @Deprecated
  def getVocabularySet(sparkContext: SparkContext, dictPath: String): collection.immutable.Set[String] = {
    if (dictPath.indexOf(CharacterUtil.CONSTANT_NONE) <= -1) {
      sparkContext.textFile(dictPath)
        .flatMap(line => line.split(CharacterUtil.CONSTANT_TAB).drop(1))
        .flatMap(line => line.split(CharacterUtil.CONSTANT_SPACE32))
        .filter(_.nonEmpty)
        .collect()
        .toSet
    } else {
